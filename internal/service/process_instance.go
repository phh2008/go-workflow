...
func (e *Engine) InstanceStart(ctx context.Context, procID int, businessID, comment, variablesJSON string) (int, error) {
	instID, startNode, err := e.instanceInit(ctx, procID, businessID, variablesJSON)
	if err != nil {
		return 0, err
	}

	err = e.startNodeHandle(ctx, instID, startNode, comment, variablesJSON)
	if err != nil {
...
// force 为 false 时，只有流程回到发起人这里才能撤销。
func (e *Engine) InstanceRevoke(ctx context.Context, instID int, force bool, revokeUserID string) error {
	if !force {
		...
		var id int
		if err := e.db.WithContext(ctx).Raw(sql, instID).Scan(&id).Error; err != nil {
			return err
		}
		if id == 0 {
	...
	// 执行流程撤销事件
	if err := e.runProcEvents(ctx, process.RevokeEvents, instID, revokeUserID); err != nil {
		return err
	}

	// 调用 endNodeHandle，做数据清理归档
	return e.endNodeHandle(ctx, instID, 2)
}
...
func (e *Engine) startNodeHandle(ctx context.Context, instID int, startNode *model.Node, comment, variablesJSON string) error {
	...
	// 完成task，并获取下一步NodeID
	if err := e.TaskPass(ctx, taskIDs[0], comment, variablesJSON, false); err != nil {
		return err
	}
